 ...
 * 2. <tree :treeData="treeData" v-model="deviceObject.sandIds" title="请选择砂场名称" :showCheckbox="true" :name="deviceObject.sandName" @getTreeCheckDialogName="getTreeCheckDialogName" :defaultProps="defaultProps"></tree>
 * 3. // 获取tree 选中节点name 向下分发name值
    getTreeCheckDialogName(name) {
...
export default {
  ...
  computed: {
    // 是否显示弹出框
    officeDialog() {
      return this.$store.state.dialog.treeDialog
    },
    checkedKeys() {
  ...
  methods: {
    // 切换弹出框
    toggleDialog() {
      this.$store.dispatch('toggleTree')
    },
    // 点击页面其他地方触发事件
    handleClose(done) {
      this.cancel()
    ...
    emitInput() {
      ...
      this.$emit('getTreeCheckDialogName', this.treeObject.nodes)
    },
    messageSubstring
  }
}
