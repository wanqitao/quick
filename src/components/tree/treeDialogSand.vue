    ...
    <el-dialog :title="title" :modal="true" :visible.sync="officeDialog" class="tree-continer-dialog" width="30%" top="10vh" :before-close="handleClose">
      ...
      <el-tree class="filter-tree" 
        ...
        <span class="custom-tree-node" slot-scope="{ node, data }" style="width:100%;">
          <span :class="{tree_active:data.id===treeObject.id}" :title="node.label">{{ messageSubstring(node.label,36) }}</span>
        </span>
      </el-tree>
 ...
 * 2. <tree :treeData="treeData" v-model="deviceObject.sandIds" title="请选择砂场名称" :showCheckbox="true" :name="deviceObject.sandName" @getTreeCheckDialogName="getTreeCheckDialogNameSand" :defaultProps="defaultProps"></tree>
 * 3. // 获取tree 选中节点name 向下分发name值
    getTreeCheckDialogNameSand(name) {
...
export default {
  ...
  computed: {
    // 是否显示弹出框
    officeDialog() {
      return this.$store.state.dialog.treeDialogSand
    },
    checkedKeys() {
  ...
  methods: {
    // 切换弹出框
    toggleDialog() {
      this.$store.dispatch('toggleTreeDialogSand')
    },
    // 点击页面其他地方触发事件
    handleClose(done) {
      this.cancel()
    ...
    emitInput() {
      ...
      this.$emit('getTreeCheckDialogNameSand', this.treeObject.nodes)
    },
    messageSubstring
  }
}
