 ...
 * 2. <tree :treeData="treeData" v-model="deviceObject.sandIds" title="请选择砂场名称" :showCheckbox="true" :name="deviceObject.sandName" @getTreeCheckPrinName="getTreeCheckPrinName" :defaultProps="defaultProps"></tree>
 * 3. // 获取tree 选中节点name 向下分发name值
    getTreeCheckPrinName(name) {
...
export default {
  ...
  computed: {
    // 是否显示弹出框
    officeDialog() {
      return this.$store.state.dialog.miningAreaDialog
    },
    checkedKeys() {
  ...
  methods: {
    // 切换弹出框
    toggleDialog() {
      this.$store.dispatch('miningAreaDialog')
    },
    // 点击页面其他地方触发事件
    handleClose(done) {
      this.cancel()
    ...
    emitInput() {
      ...
      if (this.isObject) {
        this.$emit('getTreeCheckPrinName', this.treeObject.node)
      } else {
        this.$emit('getTreeCheckPrinName', this.treeObject.name)
      }
