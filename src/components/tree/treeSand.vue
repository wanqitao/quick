 ...
 * 2. <tree :treeData="treeData" v-model="deviceObject.sandIds" title="请选择砂场名称" :showCheckbox="true" :name="deviceObject.sandName" @getTreeCheckSandName="getTreeCheckSandName" :defaultProps="defaultProps"></tree>
 * 3. // 获取tree 选中节点name 向下分发name值
    getTreeCheckSandName(name) {
...
export default {
  ...
  computed: {
    // 是否显示弹出框
    officeDialog() {
      return this.$store.state.dialog.sandDialog
    },
    checkedKeys() {
  ...
  methods: {
    // 切换弹出框
    toggleDialog() {
      this.$store.dispatch('toggleSand')
    },
    // 点击页面其他地方触发事件
    handleClose(done) {
      this.cancel()
    ...
    emitInput() {
      ...
      if (this.isObject) {
        this.$emit('getTreeCheckSandName', this.treeObject.node)
      } else {
        this.$emit('getTreeCheckSandName', this.treeObject.name)
      }
